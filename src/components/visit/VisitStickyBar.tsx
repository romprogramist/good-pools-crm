...
export function VisitStickyBar({
  ...
  const canComplete = checklistOk && photoOk && !pending;
    ...
    <div className="sticky bottom-0 left-0 right-0 z-30 border-t border-zinc-200 bg-white/95 px-4 py-3 backdrop-blur">
      <div className="mx-auto flex max-w-2xl items-center justify-between gap-3">
        <div className="text-xs text-zinc-600">
          <div>
            Чек-лист:{" "}
            <span className={checklistOk ? "text-green-600" : "text-red-600"}>
              {checklistFilled}/{checklistTotal}
            </span>
          </div>
          <div>
            Фото:{" "}
            <span className={photoOk ? "text-green-600" : "text-red-600"}>
              {photoCount}
            </span>
          </div>
          <div>
            Сумма: <span className="font-medium">{total.toLocaleString("ru-RU")} ₽</span>
          </div>
        </div>
